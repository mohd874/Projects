...
public class SpaceBlasterGame extends ArcadeGame  implements SensorEventListener
{ 
	// Game name
	...
	// positions for orintation sensor
	float xSensor = getWidth()/2;
	float ySensor = getHeight() - 20;
...
//	}
	...
	public void moveShip() {
		...
		y = (int) ySensor;
		if(x < 0)x = 0;
		if(x > getWidth() - ship.getWidth())x = getWidth() - ship.getWidth();
		
		if(y < 0)y = 0;
		if(y > getHeight() - ship.getHeight())y = getHeight() - ship.getHeight();
	}
	...
	public void onSensorChanged(SensorEvent event) {
		xSensor = -0.1F * event.values[1];
		ySensor =  0.1F * event.values[2];
	}
}

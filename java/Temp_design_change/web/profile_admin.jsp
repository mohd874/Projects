        ...
        <div class="ndesign" id="cssall">
            ...
            <div class="ndesign" id="ncenter">
                ...
                <sql:transaction dataSource="${myDB}">
                    ...
                    <sql:query var="names">
                        select user_id, name,surname, usertype.user_type from sysuser, usertype where usertype.type_id = sysuser.user_type
                        order by name
                    </sql:query>
                ...
                <form id="form2" method="post" action="profile_admin.jsp">
                    <p>Select a customer:
                        <select name="user_id">
                            <option value=""> SELECT </option>
                            <c:forEach var="user" items="${names.rows}">
                                <option value="${user.user_id}" <c:if test="${param.user_id == user.user_id}">selected </c:if> > ${user.name} ${user.surname}, ${user.user_type} </option>
                            </c:forEach>
                        </select> 
                    </p>
                    <p>
